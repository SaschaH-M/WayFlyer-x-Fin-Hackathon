...
export default function StoryMode() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => { if (i < STEPS.length - 1) { setI(i + 1); setProgress(0); } else stop(); }, total);
    return () => { clearTimeout(t); clearInterval(iv); };
  }, [on, auto, i]);

  const start = () => { setI(0); setOn(true); setAuto(true); setProgress(0); };
  const stop = () => { setOn(false); };

  if (!on) return (
    <button onClick={start} title="Auto-play the 3-minute story"
      style={{ position: "fixed", right: 22, top: 22, zIndex: 300, fontFamily: "inherit", fontSize: 13, fontWeight: 700,
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
            <button className="chip" onClick={() => { setI(Math.max(0, i - 1)); setProgress(0); }} disabled={i === 0}>‹ Back</button>
            <button className="chip" onClick={() => setAuto((a) => !a)}>{auto ? "⏸ Pause" : "▶ Auto"}</button>
            <button className="chip" onClick={() => (i < STEPS.length - 1 ? (setI(i + 1), setProgress(0)) : stop())}>{i < STEPS.length - 1 ? "Next ›" : "Finish"}</button>
        ...
        <div style={{ display: "flex", gap: 4, marginTop: 12 }}>
          {STEPS.map((step, j) => {
            let fill = 0;
            if (j < i) fill = 1;
            else if (j === i) fill = progress;
            return (
              <div key={j} style={{
                flex: 1, height: 4, borderRadius: 2, position: "relative", overflow: "hidden",
                ...
                <div style={{
                  ...
                  background: "var(--bl)",
                  width: `${fill * 100}%`,
                  transition: j === i ? "width .3s linear" : ".3s",
                }} />
              </div>
            );
          })}
        </div>
        <div style={{ textAlign: "center", marginTop: 4, fontSize: 10, color: "var(--t3)" }}>
          {auto ? `${Math.ceil((1 - progress) * s.secs)}s remaining` : "Paused"}
        </div>
      </div>
    </div>
  );
